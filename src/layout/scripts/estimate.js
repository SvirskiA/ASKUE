...
let purchase_NShAL = 0; // Приобретение и монтаж наконечника НШАЛ 16-14  на жилу СИП-4и
let purchase_n_montage_TUT = 0; // Пиобретение и монтаж термоусаживаемой трубки ТУТ 35/15
let purchase_PKV = 0; // Приобретение пасты кварце-вазилиновой (ПКВ)
let zadelka_SIP = 0; // Заделка провода СИП-4и
...
for (let i = 0; i < userInput.length; i += 1) {
  ...
  purchase_NShAL += worksDataCollect(userInput[i]).purchase_NShAL * userInput[i].countOfSEM;
  purchase_n_montage_TUT += worksDataCollect(userInput[i]).purchase_n_montage_TUT * userInput[i].countOfSEM;
  purchase_PKV += worksDataCollect(userInput[i]).purchase_PKV * userInput[i].countOfSEM;
  zadelka_SIP += worksDataCollect(userInput[i]).zadelka_SIP * userInput[i].countOfSEM;
...
if (purchase_NShAL) {
  table.append(createRow('Приобретение и монтаж наконечника НШАЛ 16-14 на жилу СИП-4и', 'шт.',
    purchase_NShAL.toFixed(2)));
}
if (purchase_n_montage_TUT) {
  table.append(createRow('Пиобретение и монтаж термоусаживаемой трубки ТУТ 35/15', 'м',
    purchase_n_montage_TUT.toFixed(2)));
}
if (purchase_PKV) {
  table.append(createRow('Приобретение пасты кварце-вазилиновой (ПКВ)', 'г',
    purchase_PKV.toFixed(2)));
}
if (zadelka_SIP) {
  table.append(createRow('Заделка провода СИП-4и', 'шт.',
    zadelka_SIP.toFixed(2)));
}
